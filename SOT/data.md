...
- 2026-04-26 — added `data/building_focus_extended.json` covering all 50 tier-2 companies (rank 31–80) with **75 building entries**: 50 HQs + 25 secondary hubs (extra SF/NYC/London/Tokyo/Beijing offices for Salesforce, Meta, Tesla, AMD, Goldman Sachs, BNY, TikTok, Wells Fargo, etc., to address the SF scope-mode cycling issue). Compiled from Wikipedia infoboxes, company contact pages, and live WebSearch against business directories (D&B, GlobalData, Craft.co). All entries are "high" or "medium" confidence except 2 "low" (ElevenLabs London exact address, Nebius Amsterdam exact address); zero fabrications.
- 2026-04-27 — `src/buildings.js` now merges both JSON files into a single `buildings` array. Lookup helpers: `findBuildingForOffice(office)` requires EXACT (company, city) match — no HQ fallback (so Amazon Paris polyline stays in Paris, not stacked on Seattle); `bestBuildingForCompany(name)` returns the company's HQ entry for TARGETS-rail clicks (so EY/Ro/Deloitte/Binance always reach scope even when their busiest office is in a city without building data); `findOfficeForBuilding(building)` does the inverse lookup to map a scoped building → matching office row (used by scope.js to skip empty brackets and to power click-bracket-to-open-panel).
- 2026-04-27 — coverage stat: 127 building entries match **115 of 4,682 offices = 2.5% scope-view coverage**. The other 4,567 offices use Nominatim city geocoding. Re-geocoding the long tail is the highest-value next data task — Google Places API on the same GCP project (`669037656528`) is the recommended path (~$15-20 estimated; ~30 min script).
