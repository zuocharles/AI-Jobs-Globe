...
- 2026-04-26 — added `VITE_GOOGLE_MAPS_API_KEY` to `.env` and to all 3 Vercel environments (Production/Preview/Development). Map Tiles API enabled on GCP project `669037656528`. Daily quota cap recommended in Google Cloud Console (~$5/day alert, ~$20/day hard cap) to bound viral-spike costs.
- 2026-04-27 — frontend logic split between `focusOffice` (polyline-click path: strict city match, never lies about location) and `focusCompany` (TARGETS-rail / scope-dropdown / city-pill paths: uses HQ fallback so the click always reaches scope when any building data exists). Both call into `enterScope()` from `src/scope.js`, which now ALSO owns the right-side detail panel for the duration of scope mode — every `enterScope` / `cycle` / `exitScope` automatically syncs panel content to the active building's office row.
- 2026-04-27 — `src/panel.js` `openPanel` is now sequence-guarded: a module-scope counter is incremented on every call, and stale Supabase fetch responses no-op instead of overwriting the DOM. Fixes a cold-start race where the first 10–20 s of clicking through TARGETS could leave the panel stuck on whichever fetch resolved last.
- 2026-04-27 — `startAutoSpin(viewer, isScopeActive)` in `src/globe.js` rotates the camera ~0.8°/sec around `Cartesian3.UNIT_Z` when at home view; suppressed when `isScopeActive()` returns true OR camera altitude < 10,000 km. `dt` capped at 100 ms so a backgrounded tab doesn't teleport the camera on resume.
